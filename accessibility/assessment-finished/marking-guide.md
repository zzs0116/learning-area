...
<dl>
  ...
  <dd>Two marks for this. Anything reasonable really; the ideas we had are:
    <ul>
      ...
      <li>Find a way to annouce screenreader users how many comments are displayed then the show comments control button is clicked. For example, put the <code>role</code> attribute with <code>"alert"</code> value on the <code>h2</code> element in the comments section and update its text like so "Comments (1 available)".</li>
    </ul>
  </dd>
</dl>
